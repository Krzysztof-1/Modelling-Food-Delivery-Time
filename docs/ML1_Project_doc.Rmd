...
## 2.1 Data Source
The data set used in this project originates from a real-world scenario involving food delivery logistics. It is publicly available on Kaggle @gibin2023food. This data set captures a diverse and comprehensive set of variables related to food delivery orders, making it an ideal candidate for exploring machine learning approaches to predict delivery times.

**Data Set Overview**
The data set contains 10,001 delivery records, each representing a unique food delivery transaction. These records encompass various dimensions of the delivery process, including geographic information, courier characteristics, weather and traffic conditions, and order-specific features. The data schema is outlined as follows:
...
The Figure "Restaurant and Delivery Locations" displays all restaurant and delivery locations overlaid on the background map defined by bounding box. The plotted points indicate that the observations were collected from 22 cities across India, including major urban centers such as New Delhi, Mumbai, Bangalore, Kolkata, and Chennai.


```{r geographic_filtering, fig.height=2.5, fig.align='center', warning=FALSE, message=FALSE, exec = FALSE}
...
# Figure : Plot restaurant and delivery locations on the map
ggplot() +
  ...
  labs(title = "Restaurant and Delivery Locations",
...
All columns that are neither continuous nor factor variables have been removed from the dataset.
...
df_clean <- df.food_time.clean %>%
  ...
  mutate(
    ...
    traffic_level_factor = relevel(traffic_level_factor, ref = "very low"),
    ...
    weather_category = factor(case_when(
      ...
      TRUE ~ NA_character_
    ), levels = c("Clear", "Poor Visibility", "Rainy"), ordered = FALSE),
    weather_category = relevel(weather_category, ref = "Clear"),
  ...
  select(-order_type, -vehicle_type, -weather_description, -traffic_level,-traffic_level_cleaned, -weather_description_cleaned) %>% 
  drop_na()
```
...
```{r delivery_time_eda, fig.width=8, fig.height=3, message=FALSE, warning=FALSE}
...
# Step 1: Get summary
summary_stats <- summary(df_clean$delivery_time_min)

# Step 2: Convert to data frame (1 row, 6 columns)
summary_df <- as.data.frame(t(as.matrix(summary_stats)))

# Step 3: Rename columns
colnames(summary_df) <- c("Min.", "1st Qu.", "Median", "Mean", "3rd Qu.", "Max")

# Step 4: Show as kableExtra table
summary_df %>%
  kbl(caption = "Summary Statistics for Delivery Time [min]", digits = 2, align = "c") %>%
...
```{r log_delivery_time_eda, fig.width=8, fig.height=3, message=FALSE, warning=FALSE}
...
log_summary <- summary(df_clean$log_delivery_time)

# Convert to 1-row data frame
log_summary_df <- as.data.frame(t(as.matrix(log_summary)))

# Rename columns
colnames(log_summary_df) <- c("Min.", "1st Qu.", "Median", "Mean", "3rd Qu.", "Max")

# Display with kableExtra
log_summary_df %>%
  kbl(caption = "Summary Statistics for Log-Transformed Delivery Time", digits = 3, align = "c") %>%
  kable_styling(position = "center", full_width = T, bootstrap_options = c("striped", "condensed", "hover"))
```
...
```{r distance_log_time_traffic_level, fig.width=8, fig.height=3, message=FALSE, warning=FALSE}
...
```{r ggpairs, fig.width=8, fig.height=4, message=FALSE, warning=FALSE}
...
The following chapter contains all the models which we fitted for our data set.
...
Before fitting the first model, we need to split the data into training and test sets. We do this using stratified sampling based on the factor `traffic_level_factor`. This is done because, during EDA we observed an imbalance in the `traffic_level_factor`, and it appeared to be a strong predictor. Stratified sampling ensures that this imbalance is proportionally reflected in both the training and test data sets.
...
```{r lm_model_1_summary, collapse=TRUE}
...
The R-squared value of the model is 0.9202. This indicates that **92.02% of the variance** in the log-transformed delivery time is explained by the model.
...
```{r lm_diagnostic_plots1, fig.width=12, fig.height = 6}
# diagnostic plots
...
```{r AIC_comparison_lm1_2}
#AIC(lm.delivery1)
#AIC(lm.delivery2)
# Create a data frame with model names and AIC values
aic_table <- data.frame(
  Model = c("Model 1", "Model 2"),
  AIC = c(AIC(lm.delivery1), AIC(lm.delivery2))
)

# Display the table centered
aic_table %>%
  kbl(caption = "AIC Comparison of Model 1 and Model 2", align = "c") %>%
...
In the next step, we explore whether an interaction exists between `distance_km` and `traffic_level_factor`. We choose to do so, because the effect of distance on delivery time may be different / stronger under heavy traffic conditions. For example, slow progress over a long distance could lead to much longer delivery times.
...
formulas <- data.frame(
  ...
  Formula = c("log_delivery_time ~ distance_km * traffic_level_factor + order_type_factor")
)

kbl(formulas, caption = "Model Formula", align = "l") %>%
  kable_styling(position = "center", full_width = T)
```
...
```{r lm_interaction, collapse=TRUE}
set.seed(123)
...
aic_table2 <- data.frame(
  Model = c("Model: delivery2", "Model: interaction"),
  AIC = c(AIC(lm.delivery2), AIC(lm.interaction))
)

# Display the table centered
aic_table2 %>%
  kbl(caption = "AIC Comparison: delivery2 vs interaction", align = "c") %>%
  kable_styling(position = "center")
```
...
# Perform ANOVA and convert result to a tidy data frame
anova_results <- anova(lm.delivery2, lm.interaction)
anova_df <- tidy(anova_results)

# Replace model formulas with simple names
anova_df$term <- c("delivery2", "interaction")
colnames(anova_df)[colnames(anova_df) == "term"] <- "Model"

# Display in styled kableExtra table
anova_df %>%
  kbl(
    caption = "ANOVA Comparison: delivery2 vs interaction",
    digits = 4,
    align = "c"
  ) %>%
  kable_styling(position = "center", full_width = TRUE, bootstrap_options = c("striped", "condensed", "hover"))
...
After selecting `lm.interaction` as our final model, we now evaluate its performance on the test data set.
...
```{r lm_predicions_plot, fig.width=12, fig.height = 6}
...
```{r lm_rmse_rsquared_calc}
# calculation root mean squared error and r_squared on test data
rmse_lm3 <- sqrt(mean((predicted_min - actual_min)^2))
...
metrics <- data.frame(
  ...
  Value = c(rmse_lm3,r_squared)
)

# Display as kableExtra table
metrics %>%
  kbl(caption = "Model Performance Metrics", digits = 4, align = "c") %>%
  kable_styling(position = "center")
```
...
We also calculated two common evaluation metrics, the Root Mean Squared Error (RMSE) and the R-squared. The RMSE was **5.59 minutes** and the R-squared **0.883**. This tells us that the average prediction error is around 5.8 minutes and that the model explains 88.3% of the variance in delivery times on unseen data. This R² is only slightly below the training R², which suggests no overfitting.
...
After first modeling with a linear model, we decided to conduct further analysis on delivery speed. It is important to understand which variables significantly impact on delivery speed: is it the weather conditions, traffic conditions, or even the type of vehicle? The variable `average_speed_kmph` is derived by dividing ``distance_km` by `delivery_time_min`, and then converted to an integer. In this case, `average_speed_kmph` meets the requirement for a Poisson model, which assumes count data as the response variable.

<br>

#### 3.2.1.1 Modeling

Before starting the modeling, the `traffic_level_factor` variable is converted to an unordered factor, and its reference level is set to `Very Low`, so that all other traffic levels are compared against this baseline.
...
The results of the model summary show that the p-values for `courier_age_years`, `precipitation_mm`, and `vehicle_type_factor` are greater than 0.05, suggesting that they do not contribute significantly to the model. Therefore, they will not be considered in the second round of modeling.


```{r, poisson_m1, message = FALSE, warning = FALSE, collapse=TRUE}
...
```
The second attempt of the Poisson model involves a reduced set of predictors: only `temperature_celsius`, `humidity_percent`, `distance_km`, `traffic_level_factor`, and `weather_category` are included. These predictors show p-values far below 0.05, which indicates that they contribute to the model. 

Another important improvement lies in the **AIC** values: the reduced model yields a slightly lower AIC, decreasing from 51,224 to 51,219, suggesting a marginally better model fit with fewer variables. The Akaike Information Criterion (AIC) is a measure used to compare the quality of statistical models, particularly for models fitted to the same data set. It helps identify the model that best balances goodness of fit and model simplicity (@wikipedia2025aic).

Additionally, the **residual deviance** slightly increases from 7,492.1 to 7,497.7. While lower deviance generally indicates a better fit (closer to the saturated model), more parameters tend to reduce residual deviance. Therefore, a small increase does not automatically justify a more complex model (@ucla_poisson_residual_deviance; @statology_residual_deviance; @stackexchange_poisson_deviance_df).
...
$$
\begin{aligned}
\text{average_speed_kmph} = \exp(2.627 &- 0.006 \cdot \text{temperature_celsius} \\
&- 0.002 \cdot \text{humidity_percent} 
+ 0.036 \cdot \text{distance_km} \\
&+ 0.212 \cdot \text{traffic_level_factor}_{\text{Low}}
+ 0.275 \cdot \text{traffic_level_factor}_{\text{Moderate}} \\
&+ 0.162 \cdot \text{traffic_level_factor}_{\text{High}}
- 0.143 \cdot \text{traffic_level_factor}_{\text{Very High}} \\
&- 0.046 \cdot \text{weather_category}_{\text{Poor Visibility}}
- 0.072 \cdot \text{weather_category}_{\text{Rainy}})
\end{aligned}
\
$$
...
The variable `traffic_level_factor` is an unordered factor with `traffic_level_factorVery Low` as the reference category. `traffic_level_factorLow` has an exponentiated coefficient of 1.24, suggesting increase 24% in speed compared to `traffic_level_factorVery Low`. `traffic_level_factorModerate`: 1.32, 32% increase 
`traffic_level_factorHigh`: 1.18, 18% increase
`traffic_level_factorVery High`: 0.87, a 13% decrease in speed. 
These results indicate that mild to moderate traffic conditions are associated with faster delivery speeds. This could be because drivers do not rush when the traffic is very low but are compelled to speed up when traffic gets worse. Also it is possible they get more orders at traffic peak hours and need to speed up to manage. "Very High" traffic reduces average speed significantly, as expected.

For the `weather_category`, `weather_categoryClear` was taken as the baseline.
`weather_categoryPoor Visibility` results in an exponentiated coefficient of 0.96, indicating about 4% decrease in speed and `weather_categoryRainy` in exponentiated coefficient 0.93, suggesting 7% decrease comparing to the baseline. These results quantitatively describe how adverse weather conditions reduce delivery speed, with rain having a slightly greater negative effect than poor visibility.
...
The resulting boxplot below shows the distribution of delivery speeds for each traffic level, separated by real (observed) and simulated (model-generated) speeds.
...
```{r, poisson_CP_1, fig.width=8, fig.height=4, fig.align='center'}
...
```{r, poisson_RA_1, fig.width=8, fig.height = 3}
...
# display the values in kable table
kable(overdisp_table, caption = "Overdispersion Check Result", align = "c") %>%
  kable_styling(
    full_width = TRUE,
...
``` {r binomial_model1, collapse=TRUE}
...
contingency_tbl <- table(df_clean_binomial$long_delivery_flag, df_clean_binomial$traffic_level_factor)

# Convert to data frame
contingency_df <- as.data.frame.matrix(contingency_tbl)

# Add row names as a column
contingency_df <- cbind("Long Delivery Flag" = rownames(contingency_df), contingency_df)
rownames(contingency_df) <- NULL

# Display as a kableExtra table
contingency_df %>%
  kbl(
    caption = "Contingency Table: Long Delivery Flag vs. Traffic Level Factor",
    align = "c"
  ) %>%
  kable_styling(position = "center", full_width = TRUE, bootstrap_options = c("striped", "hover", "condensed"))
...
```{r binomial_model2, collapse=TRUE}
...
```{r binomial_model3, collapse=TRUE}
...
conf_matrix <- table(Predicted = test_data$y_pred, Actual = test_data$long_delivery_flag)
...
accuracy <- mean(test_data$y_pred == test_data$long_delivery_flag)

# Display confusion matrix
conf_df %>%
  kbl(caption = "Confusion Matrix", align = "c") %>%
  kable_styling(position = "center", full_width = TRUE, bootstrap_options = c("striped", "condensed", "hover"))

# Display accuracy
data.frame(Accuracy = round(accuracy, 4)) %>%
  kbl(caption = "Binomial Model Accuracy", align = "c") %>%
...
+ 7.514 \cdot \text{order_type_factor}_{\text{Drinks}} \\
&- 0.719 \cdot \text{order_type_factor}_{\text{Meal}}
+ 0.711 \cdot \text{order_type_factor}_{\text{Snack}} \\
&- 1.732 \cdot \text{traffic_level_factor}_{\text{Low}}
+ 1.792 \cdot \text{traffic_level_factor}_{\text{Moderate}} \\
&+ 10.219 \cdot \text{traffic_level_factor}_{\text{High}}
+ 16.605 \cdot \text{traffic_level_factor}_{\text{Very High}} \\
&+ 0.448 \cdot \text{weather_category}_{\text{Poor Visibility}}
+ 1.105 \cdot \text{weather_category}_{\text{Rainy}})
\end{aligned}
\
$$
...
# Display the ANOVA comparison table
kable(anova_table, digits = 2, align = 'c', caption = "ANOVA Comparison: gam.model.test1 vs gam.model.test2") %>%
...
```{r GAM_model_3_1, collapse=TRUE, out.width = "100%", fig.height = 2}
...
```{r, GAM_model_3_2, collapse=TRUE, out.width = "100%", fig.height = 2}
...
plot(gam.model.test6, select = 1, se = TRUE, rug = TRUE, main = "Test6: Temperature Smooth Term")
...
# Plot each smooth term individually
plot(gam.model.test8, select = 1, se = TRUE, rug = TRUE, main = "Test8: Temperature Smooth Term")
...
- GAM without smoothing terms (`gam.model.test1`) – A simple GAM without smoothing terms, treating all predictors as linear, which is equal to a linear model.
...
- GAM with Log-Transformed Target (`gam.model.logtest8`) – applied to a log-transformed delivery time to account for potential skewness and stabilize variance.
...
```{r, GAM_MCE_1}
#  ---- For GAM without smoothing terms  ----
set.seed(99)
...
model_comparison <- data.frame(
  Model = c("GAM without smoothing terms (test 1)", "GAM on Original Scale (test8)", "GAM with Log-Transformed Target (logtest8)"),
...
model_comparison %>%
  kbl(caption = "Model Comparison Table", digits = 3, align = "lcc") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = TRUE)
```

**Visualization of Model Residuals and Predictions**

The figure below compares the residuals and predicted vs. actual values for each model:

Top row: Residual plots for the GAM without Smooth, GAM (Original scale) with Smooth, and GAM (Log-Transformed) with Smooth. The residuals from the GAM without Smooth show a noticeable funnel shape and heteroscedasticity, while the GAMs (especially the log-transformed one) show more stable variance and better distribution around zero.

Bottom row: Actual vs. predicted values for the same three models. The GAM without Smooth exhibits greater deviation from the 45-degree line (red), indicating less accurate predictions. Both GAM variants hug the diagonal more closely, with the Log-Transformed GAM showing the tightest clustering around the line, confirming its superior predictive performance.

These visual diagnostics reinforce the numerical results: the Log-Transformed GAM provides both better calibration and reduced error, highlighting the importance of handling non-linear relationships and skewed target distributions.

```{r, GAM_MCE_2, out.width = "100%", fig.height = 2}
# Predicted values
...
After fine-tuning and comparison, the fitted Generalized Additive Model (GAM) is specified as: 

$$
\begin{aligned}
\text{delivery_time_min} =\exp(\ & 3.272  + 0.277 \cdot \text{traffic_level}_{\text{Low}} + 0.507 \cdot \text{traffic_level}_{\text{Moderate}} \\ &+ 0.741 \cdot \text{traffic_level}_{\text{High}} 
+ 1.002 \cdot \text{traffic_level}_{\text{Very High}} 
+ 0.030 \cdot \text{weather}_{\text{Poor Visibility}} \\
&+ 0.060 \cdot \text{weather}_{\text{Rainy}} 
- 0.000249 \cdot (\text{temperature_celsius} \times \text{humidity_percent}) \\
&+ f_1(\text{temperature_celsius}) 
+ f_2(\text{humidity_percent}) 
+ f_3(\text{distance_km}) 
+ \varepsilon)
\end{aligned}
$$
The model indicates that traffic level has the most substantial impact on delivery time: as traffic worsens from "low" to "very high," delivery time increases significantly, with "very high" traffic associated with exp(1.002)≈ 2.724 → 172.4% longer times compared to "normal" traffic, and with "high" traffic associated with exp(0.741)≈ 2.098 → 109.8% longer times, and with "moderate" traffic with exp(0.507)≈ 1.660× → 66.0% longer times. Weather conditions also influence delivery time, rainy weather and poor visibility lead to modest increases of about exp(0.060)≈1.062 → 6% longer and slight increase exp(0.030)≈1.030 → 3% longer, respectively. The interaction between temperature and humidity has a small but significant negative effect, suggesting that under certain combined conditions (e.g., hot and humid), delivery time may slightly decrease. Additionally, the smooth terms for temperature, humidity, and distance reveal complex, non-linear relationships with delivery time, highlighting that these factors do not influence delivery speed in simple straight-line ways but may have thresholds or diminishing effects.
...
We started with a basic SVM model using `distance_km` and `average_speed_kmph` and applied a linear kernel with a `cost` parameter of 10. To avoid overfitting, we scaled the input variables and use the `e1071::svm()` function. Later, we will compare this model to a radial SVM and explore parameter tuning.

```{r svm_model1, collapse=TRUE}
set.seed(123)
...
We evaluated the prediction accuracy using a confusion matrix. We got an accuracy of 0.8416, meaning 84.16% of the test set was correctly classified. The 95% confidence interval for the accuracy lies between 82.3% and 85.8%. The model performs significantly better than random guessing or choosing the most frequent class. However, the model struggles to distinguish between `"moderate"` and `"high"` traffic levels. In total 49 `"moderate"` observations were misclassified as `"high"` and 53 `"high"` observations were misclassified as `"moderate"`. This pattern is also reflected visually in the decision boundaries (see next section).
...
```{r svm_model1_vis, fig.align='center', fig.width=4, fig.height=4}
...
```{r svm_model2_vis, fig.align='center', fig.width=4, fig.height=4}
...
Hyperparameter tuning is done on the SVM Model with the radial kernel since more hyperparameters can be tuned. We expect many possibilities to find a better performing model.

```{r svm_hyperparameter_tuning, cache = TRUE, eval=TRUE, collapse=TRUE}
...
The output lists all combinations of `cost` and `gamma` along with their cross-validated error metric. The combination with the **lowest error** is automatically selected as the best model.

We now extract this model and evaluate it on the **test dataset**.

```{r output_tuning, eval=TRUE, collapse=TRUE}
# Extract best model
...
In this chapter a simple neural network is used to predict delivery time based on the `log_delivery_time`. This is because prior attempts using `delivery_time_min` were not successful.
...
First step is preprocessing the data further for the neural network model. This includes removing the "bicycle" observations from `vehicle_type_factor` as they are too scarce and could negatively impact on model fitting. Next, the features and target are defined.
...
A random seed is set for consistency and the `neuralnet` package is used to make the neural network model. There are eighteen nodes in the input layer, three nodes in the single hidden layer and one node in the output layer. This configuration is chosen to maximise on the data's potential while keeping the model fairly simple.

```{r, neural_network_model, fig.width=10, fig.height=5, fig.align="center"}
set.seed(42)
...
plot(delivery_time_model, rep = "best")
```

### Evaluation

* The model has an RMSE of approx. **5.4 minutes**.

* R squared is approx. **90%** which is very good. This shows the features explain almost all the variance in the data.

* The chart shows that predicted values have a strong relationship with actual values. The model is a little biased for high values and tends to underestimate them when predicting.

```{r, neural_network_evaluation, fig.height = 3, fig.align="center"}
# Make predictions on test data
pred_scaled <- compute(delivery_time_model, cbind(test_scaled_numeric, test_encoded_categorical))

# Convert to minutes
pred_original <- exp(pred_scaled$net.result * (max_target - min_target) + min_target)

#Plot predicted vs. actual values
plot(test$delivery_time_min, pred_original, col='blue', pch=16, ylab = "Predicted delivery time", xlab = "Actual delivery time")
abline(0,1)

# RMSE
RMSE = sqrt(mean((test$delivery_time_min - pred_original)^2))

# R-squared calculation
ss_res <- sum((test$delivery_time_min - pred_original)^2)
ss_tot <- sum((test$delivery_time_min - mean(test$delivery_time_min))^2)
r_squared <- round(1 - (ss_res / ss_tot), 4)

kable(data.frame(RMSE=RMSE, "R Squared" = r_squared), caption = "Neural network RMSE and R squared") %>% 
...
In summary, the model performs very well in predicting the log delivery time. As it takes approx. 20 minutes to fit, cross validation and network architecture tuning were not carried out.
...
The linear model with interaction already explained a large portion of the variance (R² ~ 88%) and achieved an RMSE of 5.59 minutes on the test set. The log-transformed GAM model produced comparable results, with a slightly higher R² (88.6%) but a slightly higher RMSE (5.69 minutes). Neural network proved to be the best-performing model in terms of prediction accuracy for delivery times in this project with R² ~ 90% and RMSE of 5.4 minutes.
...
This project demonstrates that statistical and machine learning models can effectively predict food delivery times by combining geographic, traffic, weather, and order-related data. Distance and traffic level clearly emerged as being the most important factors influencing delivery time.

We found that simple, well-designed models — especially linear models with interaction terms — already provide strong predictive performance. Generalized Additive Models (GAMs) add flexibility for modeling nonlinear effects, which becomes especially useful in the case that data complexity increases. Neural networks provide even more potential in this regard although care needs to be taken to avoid overfitting.

The Poisson and binomial models presented here demonstrate how different modeling techniques can answer various business-related questions, such as predicting delivery speeds or classifying long deliveries (e.g. to pick suitable packaging). Furthermore, Support Vector Machines performed reasonably well for traffic classification although they were limited by feature overlap.

In practice, models like these could help improve estimated time of arrival (ETA) accuracy, support better resource planning, and enhance customer experience. The interesting aspect of this group work was that in everyday life, we use applications that use ETA predictions (e.g. Uber Eats). It was therefore very interesting to see what could be behind the ETA indication of such an app.

Future work could include adding real-time data (e.g. live traffic), considering more features (e.g. time of the day), or testing more advanced models.
